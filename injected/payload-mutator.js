...
/**
 * Notes feature — active file context injection.
 *
 * The Notes panel (content world) persists the active file's name + content to
 * chrome.storage.local. We mirror that value into a module-level cache here so
 * the (synchronous) hidden-prefix builder can append it to every request.
 */
// The active file is mirrored onto `window` by the content-world Notes code
// (it runs in the isolated world where chrome.storage is available; this
// MAIN-world script cannot read chrome.storage). We read it from the page.
const NOTES_ACTIVE_EVENT = "bds:notes-active-file";
let _activeFileCache = null;

function readActiveFileFromPage() {
  try {
    const w = typeof window !== "undefined" ? window : null;
    const val = w && w.__BDS_NOTES_ACTIVE_FILE__;
...
function setActiveFileCache(val) {
  _activeFileCache = val && val.name ? val : null;
}

function refreshActiveFileCache() {
  setActiveFileCache(readActiveFileFromPage());
  try {
    if (typeof window !== "undefined" && typeof window.addEventListener === "function") {
      window.addEventListener(NOTES_ACTIVE_EVENT, (e) => {
        try {
          const data =
            typeof e.detail === "string" ? JSON.parse(e.detail) : e.detail;
          setActiveFileCache(data);
        } catch (err) {
          /* ignore */
        }
      });
    ...
    console.warn("[BDS:Notes] cache refresh failed:", err);
  }
}

refreshActiveFileCache();

function buildActiveFileBlock() {
  if (!_activeFileCache || !_activeFileCache.name) return "";
  const text = String(_activeFileCache.content || "").trim();
  if (!text) return "";
  return `<BetterDeepSeek> <BDS:ACTIVE_FILE name="${_activeFileCache.name}">\nActive File (${_activeFileCache.name}):\n${text}\n</BDS:ACTIVE_FILE> </BetterDeepSeek>`;
}
...
export function buildHiddenPrefix(
  ...
  // Inject the active Notes file content as AI context (per Notes feature).
  const activeFileBlock = buildActiveFileBlock();
  if (activeFileBlock) {
    blocks.push(activeFileBlock);
  }
