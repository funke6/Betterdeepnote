...
async function run() {
  ...
  if (target === "firefox") {
    ...
    manifest.browser_specific_settings = {
      gecko: {
        id: "{a62f26c1-3ae8-4de3-9d16-92f5d279bf04}",
    ...
    // Some Firefox builds (ESR / older / Android) have background.service_worker
    // DISABLED and explicitly require the MV2-style `background.scripts` form.
    // In those builds the service_worker form fails manifest validation and the
    // add-on refuses to load ("background.service_worker is currently disabled.
    // Add background.scripts."). Our bundle is IIFE, so we must NOT set
    // type: "module".
    ...
    // Firefox MV3 does not support the 'sandbox' manifest key (Chrome-only);
    // sandbox.html is instead sandboxed via the iframe's own `sandbox` attribute.
    // Leaving the key in produces a harmless "unexpected property" manifest warning.
    if (manifest.sandbox) {
